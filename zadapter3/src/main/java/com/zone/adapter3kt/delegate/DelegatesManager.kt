...
class DelegatesManager(val adapter: DelegatesAdapter<*>) {
    var delegates = SparseArray<ViewDelegate<*,BaseHolder<RecyclerView.ViewHolder>>>()
    internal val FULLUPDATE_PAYLOADS = Collections.EMPTY_LIST
    val oriViewTypeMap: HashMap<Int, Int> = HashMap()

    private fun findOriViewType(itemViewType2: Int, viewBaseHolder: BaseHolder<RecyclerView.ViewHolder>? = null): Int {
    ...
    fun registerDelegate(viewType: Int, delegate: ViewDelegate<*,BaseHolder<RecyclerView.ViewHolder>>) {
    ...
    fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseHolder<RecyclerView.ViewHolder> {
    ...
    fun onBindViewHolder(position: Int, @NonNull item: Any, viewBaseHolder: BaseHolder<RecyclerView.ViewHolder>, payloads: List<Any>?) {
    ...
    fun onViewRecycled(@NonNull viewBaseHolder: BaseHolder<RecyclerView.ViewHolder>) {
        val viewType = findOriViewType(viewBaseHolder.itemViewType, viewBaseHolder)
    ...
    fun onFailedToRecycleView(@NonNull viewBaseHolder: BaseHolder<RecyclerView.ViewHolder>): Boolean {
        val viewType = findOriViewType(viewBaseHolder.itemViewType, viewBaseHolder)
        val delegate = getDelegateNoMap(viewType)
                ?: throw NullPointerException("No delegate found for " + viewType)
    ...
    fun onViewAttachedToWindow(viewBaseHolder: BaseHolder<RecyclerView.ViewHolder>) {
        val viewType = findOriViewType(viewBaseHolder.itemViewType, viewBaseHolder)
        val delegate = getDelegateNoMap(viewType)
                ?: throw NullPointerException("No delegate found for " + viewType)
    ...
    fun onViewDetachedFromWindow(@NonNull viewBaseHolder: BaseHolder<RecyclerView.ViewHolder>) {
        val viewType = findOriViewType(viewBaseHolder.itemViewType, viewBaseHolder)
        val delegate = getDelegateNoMap(viewType)
                ?: throw NullPointerException("No delegate found for " + viewType)
    ...
    fun getDelegateNoMap(viewType: Int): ViewDelegate<*,BaseHolder<RecyclerView.ViewHolder>>? {
        return delegates.get(viewType)
    }

    fun getDelegateMap(itemViewType: Int, viewBaseHolder: BaseHolder<RecyclerView.ViewHolder>? = null): ViewDelegate<*,BaseHolder<RecyclerView.ViewHolder>>? {
